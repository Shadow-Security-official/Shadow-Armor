...
- `controls/`: 162 controls in 12 files, one per hardening pillar.
- `libraries/`: effective-state resources (`sdw_sshd`, `sdw_sysctl`, `sdw_unit`, `sdw_auditd`, `sdw_pam`, `sdw_sudoers`, `sdw_kv`, `sdw_mount`, `sdw_block`, `sdw_crypto`, `sdw_mfa`...). Properties named `runtime*` read the live state, `persistent*` the state after reboot.
